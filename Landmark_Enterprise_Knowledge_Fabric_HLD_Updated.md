...
7. **AI-driven query planning (engine routing + query reframing) — design resolved 2026-08-20, see §8.1 step 0.** (Earlier text here said "design not started"; corrected — the design itself is done, only the items below remain.) Open questions: how this interacts with query caching (a reframed query defeats a naive cache-by-raw-query-string strategy); latency/cost budget once Slice 2 has real traffic to measure against.
...
9. **⚠️ Authorization is NOT enforced in Slice 2 (retrieval) — confirmed scope decision, 2026-08-20, standing open item.** Slice 2's query API (`/query`) does not apply the HARD authz pre-filter from §8.1 step 2 at all — no stub token, no filter query. This is a deliberate scope cut for the local single-user POC, not an oversight, but it is a real gap against this HLD's own non-negotiable rule ("early-binding authorization... enforced in code," §15/§6). Guardrails in place: every `/query` response carries an explicit `authorization: not_enforced_slice2` field (not just a log line), and the service logs a startup warning. **This item must be resolved before any multi-user deployment or before real RBAC/classification (a later slice) ships** — track it there explicitly rather than assume it gets picked up implicitly. Full detail: `Landmark_Knowledgebase_Slice2_Retrieval_Design.md` §2.
